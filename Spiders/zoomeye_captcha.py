...
def Zoomeye():
    ...
    plt.show()
    print('正在识别验证码...')
    result = main('***请填入你的联众账号***', '***请填入你的联众密码***', 'captcha.jpg',
                  'http://v1-http-api.jsdama.com/api.php?mod=php&act=upload', '', '', '1001', '')  # 四位英数混合
    print(result)  # 返回实例: {"data":{"val":"3752","id":21708338617},"result":true}
    captcha = ''
    if result.get('result') == True:
        captcha = result.get('data').get('val')
        print('识别验证码成功!验证码为:' + captcha)
    else:
        print('验证码识别失败!')
    # 3.post提交账号密码,打码登录
    url = 'https://sso.telnet404.com/cas/login/'
